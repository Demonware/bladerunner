"""Some unit tests for Bladerunner's subnet conversion utilities."""
...
class TestSubnetConversion(unittest.TestCase):
    def test_simple_small(self):
        ...
        self.assertEqual(bladerunner.ips_in_subnet(subnet), should_be)

    def test_expanded_subnet(self):
        starting = "10.17.29.128/255.255.255.252"
        self.assertIn("10.17.29.130", bladerunner.ips_in_subnet(starting))

    def test_converting_back_and_forth(self):
        starting = "10.1.2.3"
        made_binary = bladerunner._ip_to_binary(starting)
        self.assertEqual(bladerunner._binary_to_ip(made_binary), starting)

    def test_ip_with_no_mask_returns_none(self):
        starting = "1.2.3.4"
        members = bladerunner.ips_in_subnet(starting)
        self.assertEqual(members, None)

    def test_big_network(self):
        starting = "1.2.3.4"
        members = bladerunner.ips_in_subnet("{}/16".format(starting))
        self.assertIn(starting, members)

    def test_slash_thirtytwo(self):
        starting = "192.168.16.5/32"
        should_be = ["192.168.16.5"]
        self.assertEqual(bladerunner.ips_in_subnet(starting), should_be)
    ...
    def test_invalid_ips_returns_none(self):
        ...
        self.assertEqual(bladerunner.ips_in_subnet(starting), None)
